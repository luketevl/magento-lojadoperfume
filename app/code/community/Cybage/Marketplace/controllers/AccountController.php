        ...
        try {
            ...
            if (empty($errors)) {
                ...
                if($this->getRequest()->getParam('check_seller_form')) {
					...
					$customer->setCompanyPublickeypagseguro($this->getRequest()->getPost('company_publickeypagseguro'));
            ...
            if ($e->getCode() === Mage_Customer_Model_Customer::EXCEPTION_EMAIL_EXISTS) {
                ...
                $message = $this->__('Já existe uma conta com este endereço de e-mail. Se tiver a certeza de que é o seu endereço de e-mail, <a href="%s">Click aqui</a> Para obter sua senha e acessar sua conta.', $url);
            ...
            $session->setCustomerFormData($this->getRequest()->getPost())
                ->addException($e, $this->__('Não é possível salvar o cliente.'));
        }
        $errUrl = $this->_getUrl('*/*/create', array('_secure' => true));
        $this->_redirectError($errUrl);
    }
    ...
    protected function _uploadImage($fileName,$fieldName,$customerId)
    {
        ...
        { 
            $this->_getSession()->setCustomerFormData($this->getRequest()->getPost())
                               ->addException($e, $this->__('Não é possível salvar a imagem. Verifique a permissão de proprietário e pasta.'));                              
        }
            ...
            if ($customerErrors !== true) {
                ...
                if ($this->getRequest()->getParam('change_password')) {
                    ...
                    if ($customer->hashPassword($currPass, $salt) == $oldPass) {
                        if (strlen($newPass)) {
                            ...
                            $errors[] = $this->__('O novo campo de senha não pode estar vazio.');
                        }
                    } else {
                        $errors[] = $this->__('Senha atual inválida');
                    }
                }
                $validationFlag = 0; 
                ...
                if($this->getRequest()->getParam('check_seller_form'))
                {
                    ...
                    if (isset($_FILES['company_banner']['name']) && $_FILES['company_banner']['name'] != '') 
                    {
                        ...
                        {   
                            $errors[] = $this->__('Envie o arquivo válido para o banner da empresa.');        
                        }
                    } 
                    /******************* end of company banner code ******************************** */
                    ...
                    if (isset($_FILES['company_logo']['name']) && $_FILES['company_logo']['name'] != '') 
                    {
                        ...
                        {   
                            $errors[] = $this->__('Envie o arquivo válido para o logotipo da empresa.');        
                        }
                    }
                    /******************* end of company logo code ******************************** */			   
