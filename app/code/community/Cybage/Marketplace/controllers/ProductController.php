...
class Cybage_Marketplace_ProductController extends Mage_Core_Controller_Front_Action
{
     ...
     *    Add marketplace product
     * */
    public function exibesubAction(){
    	$categsId = $this->getRequest()->getParam('strid');
    	//echo 'aqui'.$categsId;
		if($categsId!=''){
			$categsIdArray = explode(',', $categsId);
			?>
			<select name="product[category_ids][]" class="required-entry select multiselect" multiple="multiple" style="min-height:150px;">
        	<?php 
        		$category = Mage::helper('marketplace')->getCategoryCollection($categsIdArray);
    ...
    public function addAction() {
        ...
        if ($count == 0) {
            ...
            $session->addError($this->__('Não há categoria disponível, tente depois de algum tempo.'));
            $this->_redirect('*/*/');
            return;
        }
    ...
    public function softDeleteAction() {
        ...
        if ($id = $this->getRequest()->getParam('id')) {
            ...
            try {
                ...
                $session->addSuccess(Mage::helper('marketplace')->__('O produto foi removido.'));
                //logging data for product delete action with success status
                $result = 1;
                $this->saveLoggingAction($action,$result,$id);
                $this->_redirect('*/product/');
            } catch (Exception $e) {
                $session->addError($this->__('Não foi possível remover o registro.'));
    ...
    public function productImageDeleteAction() {
        ...
        if ($productId && $imageType) {
            ...
            try {
                ...
                $product->save();
                $session->addSuccess(Mage::helper('marketplace')->__('A imagem do produto foi removida.'));
    ...
    public function saveAction() {
        ...
        try {
            ...
            if ($product->getIsEditMode()) {
                $action = 2;
                $session->addSuccess($this->__("Produto atualizado com Sucesso."));
            } else {
                $action = 3;
                $session->addSuccess($this->__("Produto adicionado com sucesso."));
            }
    ...
    protected function getUploadedProductImageData($product) {
        ...
        if ($_FILES) {
            foreach ($_FILES as $key => $value) {
                ...
                if ($value['size'] > $fileSize) {
                    $this->_getSession()->addError(Mage::helper('marketplace')->__('Não foi possível processar o seu pedido devido ao tamanho de imagem grande. Tente novamente.'));
                ...
                if (isset($imageName) && (file_exists($imageTmpName))) {
                    try {
                        ...
                        $this->_getSession()->addError("O arquivo selecionado não pode ser carregado.");
                        //die($e->getMessage());
                    }
                }
            }
        ...
        foreach ($mediaArray as $imageType => $fileName) {
            ...
            if (file_exists($filePath)) {
                ...
                $this->_getSession()->addError("Arquivo de imagem Não existe ");
                $product->setIsError(true);
                return $product;
            }
        }
        ...
        if ($data) {
            try {
                ...
                if (!$import->getProcessedRowsCount()) {
                    $session->addError($this->__('O arquivo não contém dados ou sku duplicado. Carregue outro'));
                } else {
                    if (!$validationResult) {
                        if ($import->getProcessedRowsCount() == $import->getInvalidRowsCount()) {
                            $session->addNotice(
                                $this->__('O arquivo é totalmente inválido. Corrija os erros e volte a carregar o ficheiro')
                            );
                        } elseif ($import->getErrorsCount() >= $import->getErrorsLimit()) {
                            $session->addNotice(
                                $this->__('Limite de erros (%d) atingido. Corrija os erros e volte a carregar o ficheiro', $import->getErrorsLimit())
                            );
                        } else {
                            if ($import->isImportAllowed()) {
                                $session->addNotice(
                                    $this->__('Corrija os erros e volte a carregar o ficheiro ou simplesmente prima o botão "Importar" para saltar linhas com erros'),
                                    true
                                );
                            } else {
                                $session->addNotice(
                                    $this->__('O arquivo é parcialmente válido, mas a importação não é possível'), false
                                );
                            }
                        }
                        // errors info
                        ...
                        if ($import->isImportAllowed()) {
                            ...
                            $session->addSuccess(
                                $this->__('Importação concluída com êxito.'), true
                            );
                            $this->_redirect('*/*/import');
                        } else {
                            $session->addError(
                                $this->__('O arquivo é válido, mas a importação não é possível'), false
                            );
                        }
                    }
                    $session->addNotice($import->getNotices());
                    $session->addNotice($this->__('Checke as linhas: %d, checked entities: %d, linhas invalidas: %d, total de erros: %d', $import->getProcessedRowsCount(), $import->getProcessedEntitiesCount(), $import->getInvalidRowsCount(), $import->getErrorsCount()));
                }
            } catch (Exception $e) {
                $session->addNotice($this->__('Corrija os erros e volte a carregar o ficheiro'))->addError($e->getMessage());
            }
        } elseif ($this->getRequest()->isPost() && empty($_FILES)) {
            $session->addError($this->__('O arquivo não foi enviado'));
        } else {
            $session->addError($this->__('Os dados são inválidos ou o arquivo não foi enviado'));
        }
    ...
    private function _uploadZipImages($time)
    {
        if($_FILES['import_image']['name']) 
        {
            ...
            {
                $message = array('failure', 'Não foi possível processar a solicitação de upload por zip devido ao tamanho grande. Tente novamente.');
                return $message;
            }
            ...
            if(!$continue) 
            {
                $message = array('failure', 'O arquivo que você está tentando carregar não é um arquivo .zip. O upload ZIP falhou. Tente novamente com o arquivo correto.');
                return $message;
            }
            ...
            if(move_uploaded_file($source, $file)) 
            {
                ...
                $message = array('success', 'O arquivo Zip foi carregado e descompactado.');
            } else {
                $message = array('failure', 'Ocorreu um problema com o upload. Por favor, tente novamente.');
            }
    ...
    private function _validateZip($dir)
    {
        ...
        if(!empty($invalid))
        {
            $this->_getSession()->addError($this->__('Arquivos abaixo inválidos foram ignorados. <br> '.implode("<br>", $invalid)));
        }
        
        if(!empty($larger))
        {   
            $this->_getSession()->addError($this->__('Não é possível processar a solicitação de arquivos abaixo devido ao tamanho de imagem grande, tente novamente. <br> '.implode("<br>", $larger)));
        }
    }
