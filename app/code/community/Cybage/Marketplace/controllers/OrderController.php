...
class Cybage_Marketplace_OrderController extends Mage_Core_Controller_Front_Action
{
    ...
    public function addCommentAction() {
        ...
        try {
            ...
            if (empty($data['status']) ) {
                $error['status'] = $this->__('Selecione um status');
            }
            ...
            if(empty($error)) {
                if($order->hasInvoices() || $data['status'] == 'cancelled') {
                    ...
                    if($notify) {
                        ...
                        Mage::getSingleton('core/session')->addSuccess('O status do pedido foi atualizado.');
                        $this->_redirect('marketplace/account/vieworder',$arguement=array('order_id' => $orderId));
                   
                } else {
                   Mage::getModel('core/session')->addError("Não foi gerado o pedido para esta encomenda, contacte o administrador.");
                   $this->_redirect('marketplace/account/vieworder',$arguement=array('order_id' => $orderId));
                }
            } else {
                Mage::getSingleton('core/session')->addSuccess('O comentário não foi adicionado.');
    ...
    public function expoXmlAction(){
       if (Mage::getModel('customer/session')->isLoggedIn()) {
            ...
            $data[0] = array(
                $this->__("Order#"), $this->__("Nome da Conta"),
                $this->__("Status"), $this->__("Total de vendas"),
                $this->__("Montante recebido"), $this->__("A quantidade permanece"),
            );
            foreach ($orders as $order) {
                $data[] = array(
                    $order['increment_id'], $order['billname'],$order['status'],
                    round($order['Total'], 2), round($order['Montante recebido'], 2), round($order['A quantidade permanece'], 2)
                );
            }
    ...
    protected function _initShipment()
    {
        ...
        if ($shipmentId) {
            ...
            */
            if (!$order->getId()) {
                $this->_getSession()->addError($this->__('O pedido não existe mais.'));
                return false;
            }
            /**
            ...
            if ($order->getForcedDoShipmentWithInvoice()) {
                $this->_getSession()->addError($this->__('Não é possível fazer a remessa para o pedido separadamente da fatura.'));
                return false;
            }
            /**
            ...
            if ($tracks) {
                foreach ($tracks as $data) {
                    if (empty($data['number'])) {
                        Mage::throwException($this->__('O número de rastreamento não pode estar vazio.'));
                    }
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $shipmentCreatedMessage = $this->__('A remessa foi criada.');
            $labelCreatedMessage    = $this->__('A etiqueta de remessa foi criada.');
            ...
            if ($isNeedCreateLabel) {
                ...
                $responseAjax->setMessage(
                    Mage::helper('sales')->__('Ocorreu um erro ao criar o rótulo de remessa.'));
            } else {
                $this->_getSession()->addError($this->__('Não é possível salvar a remessa.'));
                $this->_redirect('*/*/new', array('order_id' => $this->getRequest()->getParam('order_id')));
            }
