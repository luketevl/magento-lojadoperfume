...
class Cybage_Marketplace_Adminhtml_SellerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function editAction()
    {
        ...
        if ($customer->getId()) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('marketplace')->__('O vendedor não existe'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            try {
				...
				$customer->setSellerSubscriber($data['seller_subscriber']);
				$customer->setCompanyDescription($data['company_description']);
                $customer->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__('Dados do vendedor atualizados com sucesso.')
                );
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('marketplace')->__('Não é possível encontrar o item para salvar'));
        $this->_redirect('*/*/');
    }
    ...
    public function massDeleteAction() {
        ...
        if (!is_array($sellerIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Selecione o vendedor(s)'));
        } else {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                            'Total de %d registros foram excluídos com êxito.', count($sellerIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massApproveAction() {
        ...
        if (!is_array($sellerIds)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                            'Total de %d registros foram aprovados com sucesso.', count($sellerIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massRejectAction() {
        ...
        if (!is_array($sellerIds)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                            'Total de %d registro (s) foram rejeitados com sucesso.', count($sellerIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
