...
class Cybage_Marketplace_Adminhtml_CommissionController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function payAction()
    {
        ...
        if ($customer->getId()) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('marketplace')->__('Vendedor não existe'));
            $this->_redirect('*/*/');
        }
    }
                ...
                if ($data['amount'] > 0) {
                    ...
                    if ($data['amount'] <= $actualdue) {
                        ...
                        Mage::getSingleton('adminhtml/session')->addSuccess(
                            Mage::helper('adminhtml')->__('Detalhes do pagamento adicionados com êxito.')
                        );
                        $this->sendTransactionalEmail(Mage::getModel('customer/customer')->load($data['seller_id']),$commission);
                        $this->_redirect('*/*/');
                        return;
                    }  else {
                        Mage::getSingleton('adminhtml/session')->addError('O montante do pagamento não excede o montante recebido.');
                        $this->_redirect('*/*/pay', array('id' => $this->getRequest()->getParam('id')));
                        return;
                    }
                } else {
                    Mage::getSingleton('adminhtml/session')->addError('Insira um número maior do que 0 em Pagamento.');
                    $this->_redirect('*/*/pay', array('id' => $this->getRequest()->getParam('id')));
                    return;
                }
            } catch (Exception $e) {
