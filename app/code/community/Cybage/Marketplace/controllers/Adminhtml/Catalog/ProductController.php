    ...
    public function massStatusandstateAction()
    {
        ...
        try {
            ...
            $this->_getSession()->addSuccess(
                $this->__('Total de %d registro (s) foram atualizados.', count($productIds))
            );
        }
            ...
            $this->_getSession()
                ->addException($e, $this->__('Ocorreu um erro ao atualizar o status do produto.'));
        }
        ...
        if ($status == Mage_Catalog_Model_Product_Status::STATUS_ENABLED) {
            if (!Mage::getModel('catalog/product')->isProductsHasSku($productIds)) {
                throw new Mage_Core_Exception(
                    $this->__('Alguns dos produtos processados não têm valor SKU definido. Preencha-o antes de executar operações nesses produtos')
                );
            }
        }
    }
