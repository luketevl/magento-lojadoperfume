...
class Cybage_Marketplace_ProductquestionController extends Mage_Core_Controller_Front_Action
{
    ...
    public function submitAction() {
        ...
        try {
            ...
            Mage::getSingleton('core/session')->addSuccess($this->__('Sua consulta foi enviada ao vendedor. O vendedor responderá o mais breve possível !!'));
        } catch (Exception $e) {
            Mage::getSingleton('core/session')->addError($this->__('Ocorreu algum problema técnico ao processar o seu pedido. Experimente depois de algum tempo.'));
        }
    ...
    public function linkAction() {
        ...
        if ($status == Mage_Catalog_Model_Product_Status::STATUS_DISABLED) {
            Mage::getSingleton('core/session')->addError($this->__('Este produto não está disponível agora.'));
    ...
    public function indexAction() {
        ...
        $this->getLayout()->getBlock('head')->setTitle($this->__('Perguntas sobre os meus produtos'));
        $this->renderLayout();
    }
    ...
    public function replyAction() {
        ...
        $this->_initLayoutMessages('catalog/session');
        $this->getLayout()->getBlock('head')->setTitle($this->__('Minha resposta de perguntas de produtos'));
        $this->renderLayout();
    }
    ...
    public function repliedAction() {
        ...
        Mage::getSingleton('core/session')->addSuccess($this->__('A resposta foi enviada com sucesso!!'));
        $this->_redirect('*/*');
    }
}
