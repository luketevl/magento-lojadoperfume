		...
		*/
		...
		foreach($valorFinal as $key => $value){
			if((int)$key!=0 && $pagSeguroKey[$key] !=''){
					//Mage::throwException($comissaoVendedorArray[$key].' 3');
					/*	
				...
				*/
				if($comissaoVendedorArray[$key] !='' && is_numeric($comissaoVendedorArray[$key])){
					$value = $value - (((float)$value / 100) * (float)$comissaoVendedorArray[$key] );
				}else{
					$value = $value - (((float)$value / 100) * (float)$comissaoRate );
				}
