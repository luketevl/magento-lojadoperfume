...
class CleverWeb_PagSeguroBoleto_Helper_Data extends Mage_Core_Helper_Data{
	const XML_PATH_PAYMENT_PAGSEGUROBOLETO_EMAIL              = 'payment/cleverweb_payb/merchant_email';
    const XML_PATH_PAYMENT_PAGSEGUROBOLETO_TOKEN              = 'payment/cleverweb_payb/token';
    const XML_PATH_PAUMENT_PAGSEGUROBOLETO_SANDBOX            = 'payment/cleverweb_payb/sandbox';
    const XML_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_EMAIL      = 'payment/cleverweb_payb/sandbox_merchant_email';
    const XML_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_TOKEN      = 'payment/cleverweb_payb/sandbox_token';
	const XML_PATH_PAYMENT_PAGSEGUROBOLETO_SELERMAIL      	= 'payment/cleverweb_payb/selermail';
	const XML_PATH_PAYMENT_PAGSEGUROBOLETO_TAXA      			= 'payment/cleverweb_payb/taxapagseguro';
	const XML_PATH_PAYMENT_PAGSEGUROBOLETO_TARIFA      		= 'payment/cleverweb_payb/tarifapagseguro';
	
	const XML_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_APPID      = 'payment/cleverweb_payb/appid';
	const XML_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_APPKEY     = 'payment/cleverweb_payb/appkey';	
	const XML_PATH_PAYMENT_PAGSEGURO_SANDBOX_APPID       = 'payment/cleverweb_payb/sandbox_appid';
	const XML_PATH_PAYMENT_PAGSEGURO_SANDBOX_APPKEY      = 'payment/cleverweb_payb/sandbox_appkey';
	
	
	//URLs PAGSeguro
	const URL_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_VZ      	= 'https://pagseguro.uol.com.br';
	const URL_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_VZ      	= 'https://sandbox.pagseguro.uol.com.br';
	const URL_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_WS      	= 'https://ws.pagseguro.uol.com.br';
	const URL_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_WS      	= 'https://ws.sandbox.pagseguro.uol.com.br';	
	const URL_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_STC      	= 'https://stc.pagseguro.uol.com.br';
	const URL_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_STC      	= 'https://stc.sandbox.pagseguro.uol.com.br';
	
	
	public function getTaxaPagseguro(){
		$taxa = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_TAXA);
	...
	public function getTarifaPagseguro(){
		$tarifa = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_TARIFA);
	...
	public function getParcelasSjuros(){
		$intParcelas = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_PARCELANJUROS);
	...
	public function getUrlStc(){
		$urlStc = self::URL_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_STC;
		if ($this->isSandbox()){
			$urlStc = self::URL_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_STC;
		}
		return $urlStc;
	}
	
	public function isSandbox(){
        return Mage::getStoreConfigFlag(self::XML_PATH_PAUMENT_PAGSEGUROBOLETO_SANDBOX);
    }
	
	
	public function getAppid(){
		$appid = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_APPID);
		if ($this->isSandbox()){
			$appid = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_APPID);
		}
		if (empty($appid)) {
            return false;
        }
	...
	public function getAppkey(){
		$appkey = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_APPKEY);
		if ($this->isSandbox()){
			$appkey = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_APPKEY);
		}
		if (empty($appkey)) {
            return false;
        }
	...
	public function getSelerMail(){
		return Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_SELERMAIL);
	}
		
	public function getToken(){
        //$this->checkTokenIntegrity();
        $token = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_TOKEN);
        if ($this->isSandbox()) {
            $token = Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_TOKEN);
        }
        if (empty($token)) {
            return false;
        }
	 ...
	 public function getMerchantEmail(){
        if ($this->isSandbox()) {
            return Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_EMAIL);
        }
        return Mage::getStoreConfig(self::XML_PATH_PAYMENT_PAGSEGUROBOLETO_EMAIL);
    }
	
	public function getURLaceitarvendedor($code){
		$url = ($this->isSandbox() ? self::URL_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_VZ : self::URL_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_VZ);
	...
	public function getAutozacao($reference,$mail){
		...
		$url = ($this->isSandbox() ? self::URL_PATH_PAYMENT_PAGSEGUROBOLETO_SANDBOX_WS : self::URL_PATH_PAYMENT_PAGSEGUROBOLETO_PRODUCAO_WS);
